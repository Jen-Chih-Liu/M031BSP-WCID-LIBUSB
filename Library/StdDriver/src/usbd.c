...
extern const uint8_t  g_HID_sOSStringDesc[18];
/**
 * @cond HIDDEN_SYMBOLS
 */
...
static volatile uint32_t g_usbd_UsbAltInterface = 0ul;
 volatile uint8_t  g_usbd_CtrlInZeroFlag = 0ul;
/**
 * @endcond
 */
...
void USBD_GetDescriptor(void)
{
    ...
    case DESC_QUALIFIER:
    {
        ...
        USBD_PrepareCtrlIn((uint8_t *)gu8QualifierDescriptor, u32Len);
        break;
    }
    /* Get BOS Descriptor */
    case DESC_BOS:
    ...
    case DESC_STRING:
    {
        ...
        if(g_usbd_SetupPacket[2] < 4ul)
        {
            ...
            break;
					...
					else if(g_usbd_SetupPacket[2] == 0xee)
					{
					USBD_PrepareCtrlIn((uint8_t *)&g_HID_sOSStringDesc,0x12);	
           break					;
